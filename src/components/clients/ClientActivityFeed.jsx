  ...
  return (
    <div className="space-y-4">
      ...
      <div className="neumorphic-raised rounded-xl p-4">
        ...
        {workflowInstances.filter(w => w.status !== 'cancelled').length === 0 ? (
          <p className="text-sm text-[#4A5568] text-center py-2">No active workflows yet.</p>
        ) : (
          <div className="space-y-2">
            {workflowInstances.filter(w => w.status !== 'cancelled').slice(0, 5).map((workflow) => (
              <Link
                key={workflow.id}
