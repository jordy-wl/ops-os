...
| 2026-03-03 | DATA-ENGINEER | P1-S2-DE-01 | Design partner walkthrough acceptance criteria included "trigger onboarding workflow" and "≥5 real events" — both assumed a workflow trigger UI existed. FE-03 (Sprint 2) shows workflow status only; there is no trigger button. A proxy design partner session generated 1 event (block.created). The walkthrough script and acceptance criteria for any future design partner session must be revised to reflect what the UI actually supports. Sprint 3 should add a "Start Onboarding" action on the block detail page before scheduling a real partner session. | prd/06-frontend-spec.md — Section: Design Partner Onboarding — Assumption: workflow trigger accessible to end user | moderate | PENDING |
